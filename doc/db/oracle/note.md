...
----------------oracle数据备份与还原---------------------------------------
-- 远程导出数据
-- exp sjsj1/Sjsj_123@192.168.1.22/xe file=d:\sjsj1.dmp owner=('sjsj1')
```
...
select a.column_name as zddm,
  ...
  left join all_tab_columns c
   ...
   and a.owner = c.owner
 where a.table_name = upper('SYS_SJGL_SJDX')
...
--恢复表
flashback table SHRMYY_HLB_GPHSZS to before drop;
--之前的视图
 select * from SYS.DBA_views
as of timestamp sysdate -1.5/24
where view_name='V_SHRMYY_HLB_HSRYXX_AKSTJ';
```
# dblink操作
```sql
--查询创建的dblink
...
tempfile 'D:\ORACLE\PRODUCT\10.2.0\ORADATA\XXXXDB/GXPTZJK_TEMP.dbf'
size 50m
autoextend on
...
logging
datafile 'D:\ORACLE\PRODUCT\10.2.0\ORADATA\XXXXDB/GXPTZJK_DATA.dbf'
size 50m
autoextend on
next 50m maxsize 20480m
extent management local;
...
--long转varchar
CREATE OR REPLACE FUNCTION LONG_TO_CHAR( sql_cur varchar2)
RETURN varchar AS
...
--long转varchar1
CREATE OR REPLACE FUNCTION LONG_TO_CHAR1( in_rowid rowid,in_owner
...
# 新增默认值
```sql
CREATE OR REPLACE FUNCTION F_XZMRZ( sql_cur varchar2)
RETURN varchar AS
text_c1 varchar2(32767);
--set serveroutput on size 10000000000;
begin
...
execute immediate sql_cur into text_c1;
text_c1 := substr(text_c1, 1, 4000);
if substr(text_c1,1,1)='''' then
  text_c1 := substr(text_c1,2,length(text_c1)-2);
END IF;
RETURN TEXT_C1;
END;
```
...
1. 在oracle11g下测试可用。
1. truncate后不能入新数据。
1. oracle10g还不行，listagg方法不支持。

```sql
--truncate测试表
select t.*,rowid from sjsj.qt_dcxt_cz t;
--truncate table qt_dcxt_cz;
--sys登陆，sql命令行进行恢复操作
@/oracle/FY_Recover_Data.pkg
set serveroutput on
exec Fy_Recover_data.recover_truncated_table('SJSJ','QT_DCXT_CZ');
--查看处理恢复后的数据
select * from sjsj.qt_dcxt_cz$$ t;
--恢复后删除临时表空间
...
```
附件：[[FY_Recover_Data.zip](common/download.do?xzms=false&id=CDCE7B8C35EE44A986BAE31B93E8E41E "FY_Recover_Data.zip")](common/download.do?xzms=false&id=CDCE7B8C35EE44A986BAE31B93E8E41E "FY_Recover_Data.zip")
# 临时表
```sql
--创建临时表，每次使用完会自动清空
...
--imp userid=sjsj/sjsj fromuser=sjsj touser=sjsj ignore=y file=d:\sjsj.dmp
...
--tkprof D:\ORACLE\PRODUCT\10.2.0\ADMIN\XXXXDB\UDUMP/XXXXdb_ora_5728.trc D:\out.txtoutputfile explain=etl/etl ;
