...
----------------oracle数据备份与还原---------------------------------------
-- 远程导出数据：exp sjsj2_dev/Sjsj_123@192.168.1.22/xe file=F:\SJSJ2_DEV.dmp owner=('sjsj2_dev')
-- exp sjsj2_dev/Sjsj_123@127.0.0.1/xe OWNER=SJSJ2_DEV,SJSJ1,KETTLE5 file=/mydata/data/sjsj/20220825/SJSJ2_DEV.dmp
-- imp userid=sjsj2_dev/Sjsj_123@192.168.1.22/xe fromuser=sjsj2_dev touser=sjsj2_dev ignore=y file=F:\data\sjsj\20220823\sjsj2_dev.dmp buffer=512000000
-- imp userid=sjsj2_dev/Sjsj_123@192.168.1.22/xe fromuser=sjsj1 touser=sjsj1 ignore=y file=F:\data\sjsj\20220823\SJSJ1.dmp buffer=512000000
-- imp userid=sjsj2_dev/Sjsj_123@127.0.0.1/xe fromuser=sjsj2_dev touser=sjsj2_dev1 ignore=y file=/mydata/data/sjsj/20220824/SJSJ2_DEV.dmp buffer=512000000
-- imp userid=sjsj2_dev/Sjsj_123@127.0.0.1/xe fromuser=sjsj1 touser=sjsj1 ignore=y file=/mydata/data/sjsj/20220823/SJSJ1.dmp buffer=512000000
-- imp userid=sjsj2_dev/Sjsj_123@127.0.0.1/xe fromuser=kettle_default touser=kettle5 ignore=y file=/mydata/data/sjsj/20220823/SJSJ1.dmp buffer=512000000
-- 远程导入数据：imp userid=sjsj/sjsj@127.0.0.1/mydb fromuser=sjsj2_dev touser=sjsj2_dev ignore=y file=F:\data\sjsj\20220727\SJSJ2_DEV.dmp buffer=512000000
-- 远程备份重要数据：exp sjsj2_dev/sjsj_123@127.0.0.1/mydb file=d:\sjsj2_dev.dmp tables=(sys_sjgl_bhsc,sys_qx_app,sys_sjgl_tyzd,sys_sjgl_dzd,sys_sjgl_grbj,sys_sjgl_sjzd,sys_sjgl_sjdx,sys_qx_jsqxgl,sys_qx_yhjsgl,sys_qx_jsxx,sys_qx_yhxx,sys_qx_qxxx,sys_sjgl_blob)


----本地会新增，线上存在的数据全部以线上为准的场景，将本地新增的插入线上库----------
--编号生成,将新增变化插入原库
insert into sjsj1.sys_sjgl_bhsc
select * from sjsj.sys_sjgl_bhsc t
where not exists (
select 1 from sjsj1.sys_sjgl_bhsc t1 where t1.id=t.id
);
--新增的应用
insert into sjsj1.sys_qx_app
select * from sjsj.sys_qx_app t
where not exists (
...
delete from sjsj1.sys_sjgl_tyzd t where exists (
select 1 from sjsj.sys_sjgl_tyzd t1 where t1.id=t.id and t1.gxsj>=t.gxsj
);
...
select 1 from sjsj.sys_sjgl_grbj t1 where t1.id=t.id and t1.gxsj>=t.gxsj
);
insert into sjsj1.sys_sjgl_grbj
select * from sjsj.sys_sjgl_grbj t
where not exists (
...
exists (
select 1 from sjsj.sys_sjgl_sjdx t1 where t1.id=t.id and t1.gxsj>=t.gxsj
);
--导入本地对象
...
exists (
select 1 from sjsj.sys_qx_jsqxgl t1 where t1.id=t.id and t1.gxsj>=t.gxsj
);
--导入本地角色权限关联
...
exists (
select 1 from sjsj.sys_qx_yhjsgl t1 where t1.id=t.id and t1.gxsj>=t.gxsj
);
--导入本地用户授权
...
exists (
select 1 from sjsj.sys_qx_jsxx t1 where t1.id=t.id and t1.gxsj>=t.gxsj
);
--导入本地角色
...
exists (
select 1 from sjsj.sys_qx_yhxx t1 where t1.id=t.id and t1.gxsj>=t.gxsj
);
--导入本地用户
...
exists (
select 1 from sjsj.sys_qx_qxxx t1 where t1.id=t.id and t1.gxsj>=t.gxsj
);
--导入本地权限
...
exists (
select 1 from sjsj.sys_sjgl_file t1 where t1.id=t.id and t1.gxsj>=t.gxsj
);
*/
--导入本地文件
...
exists (
select 1 from sjsj.sys_sjgl_blob t1 where t1.id=t.id and t1.gxsj>=t.gxsj
);
--导入本地大字段表数据
insert into sjsj1.sys_sjgl_blob
select * from sjsj.sys_sjgl_blob t
where not exists (
