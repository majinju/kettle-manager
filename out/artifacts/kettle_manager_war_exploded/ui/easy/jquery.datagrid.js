...
(function($) {
    ...
    function selectRow(jq, rowIndex, _103) {
        var dg = $.data(jq, "datagrid");
        var dc = dg.dc;
        var opts = dg.options;
        ...
        // start 默认仅能选择一行，多选勾选check
        //unselectAll(jq);
        //selectedRows.splice(0, selectedRows.length);
        //// end

        if (!_103 && opts.checkOnSelect) {
            _107(jq, rowIndex, true);
        }
        var row = opts.finder.getRow(jq, rowIndex);
        if (opts.idField) {
            _7(selectedRows, opts.idField, row);
        }
        opts.finder.getTr(jq, rowIndex).addClass("datagrid-row-selected");
        opts.onSelect.call(jq, rowIndex, row);
        _f3(jq, rowIndex);
    };
    function unselectRow(_109, _10a, _10b) {
    ...
    function unselectAll(_116, _117) {
    ...
    function _16b(_16c) {
        ...
        function _174(ids, _175) {
            for (var i = 0; i < ids.length; i++) {
                ...
                if (_176 >= 0) { (_175 == "s" ? selectRow: _107)(_16c, _176, true);
                }
            }
        };
    ...
    $.fn.datagrid = function(_182, _183) {
        ...
        return this.each(function() {
            ...
            var opts;
            if (state) {
                opts = $.extend(state.options, _182);
                state.options = opts;
            } else {
                opts = $.extend({},
                    $.extend({},
            ...
            if (state) {
                // 如果grid已经初始化了，就不自动load data，反之绑定事件加载多次，如需加载手工调用load方法
                return;
            }
            _95(this);
        });
