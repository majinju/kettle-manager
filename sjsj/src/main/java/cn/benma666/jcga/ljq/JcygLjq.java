...
public class JcygLjq extends DefaultLjq {
    ...
    */
    ...
    public Result plsc(JSONObject myParams) {
        ...
        //物理删除关系
        int c = db().update("delete from jcga_jcyg_shgx t where t.yxx='0' "
                + "and exists(select 1 from jcga_jcyg_jcxx j where j.ygbh=t.ygbh "
                + "and j.id in (#{join(sys.ids)}))",myParams);
        result.addMsg("物理删除关系数："+c);

        //后续继续进行逻辑删除操作
        c = db().update("update jcga_jcyg_shgx t set t.yxx='0' "
    ...
    public Result save(JSONObject myParams) {
        ...
        return swtj(super.save(myParams));
    }
}
