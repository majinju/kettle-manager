...
<my-dzplhc id="listPage" :fromdata="fromdata" :sjdxid="sjdxid">
    <button class="btn btn-primary btn-sm"  @click="azylbDcsj" title="按资源类别导出数据">
    	<i class="icon ion-ios-arrow-thin-down"></i> 导出数据</button>
</my-dzplhc>
<script type="text/x-tp" id="my-sjdx-lbplcz-tp">
</script>
<script type="text/x-tp" id="my-sjdx-lbcz-tp">
<span>
<a href="sjdx/list.do?dxdm=SYS_PLBD_HM_CKSJ&e_zydm={{= row.dm}}&e_sjdx={{= row.sjdx}}" 
a-oper="other" title="查看数据"><i class="icon ion-eye"></i>
</span>
</script>
<script>
function sjdxZdy(vp){
    ...
    * 按资源类别导出数据
    */
    vp.methods.azylbDcsj = function(){
    	if(this.$children[0].listPage.total == 0){
    		alertError("没有数据可以导出！");
    		return;
    	}
    	$.download("sjdx/getFile.do?e_cllx=azylbDcsj&dxdm="+this.$children[0]._data.sjdx.dxdm,{});
    }
}
