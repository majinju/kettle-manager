	...
	SelectPage.prototype.setInitRecord = function(refresh) {
		...
		if(key){
			if (typeof p.data === 'object') {
				...
				//如果默认值获取不到对应的key和value
				if(data.length==0){
					data.push({"dm":keyarr[0],"mc":keyarr[0]});
				}
...
//                    listWidth = el.result_area.outerWidth(),
                    listWidth = (el.container.width()>el.result_area.outerWidth()||!p.pagination)?el.container.width():el.result_area.outerWidth(),
                ...
                //下拉框与输入框一样宽。//jingma
                $("div.sp_result_area div.pagination>ul>li.pageInfoBox>a").width((listWidth-122)+"px");
                return {
                    top : top + 'px', left: left + 'px',width: listWidth+'px'
                };
            }
        };
	 ...
	 * @returns 
	 */
	function getPluginByThis(){
		var $this =  $(this).closest('div.sp_container').find('input.sp_input');
		return $this.data(SelectPage.dataKey);
	}
